...
def evaluate_function(auto_config):
    ...
    def regularize(model):
        ...
        for j, (name,parameters) in enumerate(model.named_parameters()):
            ...
            if any([k in name for k in key]):
                parameters.grad.data.add_(auto_config['reg']*torch.sign(parameters.data))
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            ratio = min(0.35, max(0, (epoch - args.warmup_epochs) // auto_config['ratio5_epochs'] * 0.05))
            ...
            train_metrics = train_one_epoch(
                epoch, model, baseline_model, loader_train, optimizer, train_loss_fn, args,auto_config['rec'],
    ...
    if best_metric is not None:
        ...
        return {'acc':best_metric} 


def train_one_epoch(
        epoch, model, baseline_model, loader, optimizer, loss_fn, args,rec,
...
if __name__ == '__main__':
    lexico_objectives = {}
    lexico_objectives["metrics"] = ["acc"]
    lexico_objectives["tolerances"] = {"acc": 0.02}
    lexico_objectives["targets"] = {"acc": 100}
    lexico_objectives["modes"] = ["max"]
    search_space = {
    "rec": tune.uniform(lower=0.1, upper=0.9),
    "reg": tune.loguniform(lower=1e-5, upper=2e-4),
    "ratio5_epochs": tune.randint(lower=10, upper=35),
    }
