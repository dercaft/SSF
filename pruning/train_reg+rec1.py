...
def main():
    ...
    try:
        ...
        def prune_to_ratio(data,ratio):
            sorted_arr = np.sort(data)
            threshold_index = int(ratio * len(sorted_arr))
            th = sorted_arr[threshold_index]
            return th
        
        def mask_layers_to_sanme_ratio(model, ratio):
            ths=[]
            for name, param in model.named_parameters():
                if get_pruned(param) < ratio:
                    data=param.data.detach().cpu().numpy().flatten()
                    th = prune_to_ratio(data,ratio)
                    THS[name]=th
                    ths.append(th)
                    mask =  torch.abs(param.data) < th
                    # param.data[name].requires_grad = False
                    param.data[mask]=0
            if ths:
                ths=np.array(ths)
                print('Ave_TH',np.mean(ths),'Var_TH',np.var(ths))
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            if ratio > 0.35:
                ratio = 0.35
            mask_layers_to_sanme_ratio(model, ratio)
            print('Ratio:',ratio)
