...
def main():
    ...
    try:
        def prune_to_ratio(param,ratio):
        ...
        def get_pruned_ratio(model, TH_val):
            ...
            for name, param in model.named_parameters():
                if 'ssf_scale' in name:# or 'ssf_shift' in name:
                    ...
                    ttotal = param.numel()
                    tpruned = (torch.abs(param.data) < 1e-4).sum().item()
                    total += param.numel()
                    if tpruned/ttotal >= 0.35 :
                        pruned += (0.35*ttotal)
                        mask_dict[name]=prune_to_ratio(param,0.35)
                    else:
        ...
        def adjust_TH_to_ratio(model, ratio):
            ...
            while l <= r:
                ...
                print(f'l={l:.7f},r={r:.7f},mid={mid:.7f},ratio={pruned_ratio:.7f}')
